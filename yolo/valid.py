...
def valid(datacfg, cfgfile, weightfile, outfile):
    ...
    for batch_idx, (data, target) in enumerate(valid_loader):
        ...
        for i in range(output.size(0)):
            ...
            im = cv2.imread(valid_files[lineId])
            label = open(valid_files[lineId].replace('train_x/','pred_yolo/').replace('.png', '.txt'), 'w')
            ...
            for box in boxes:
                ...
                for j in range(int((len(box)-5)/2)):
                    ...
                    if det_conf > 0.2:
                        cv2.rectangle(im, (max(0,x1),max(0,y1)), (min(width-1,x2),min(height-1,y2)), (0,0,255), 1)
                        label.write('%f %f %f %f\n' % (x1, y1, x2, y2))
            cv2.imwrite(valid_files[lineId].replace('train_x/','results_yolo/').replace('.png','_pred.png'), im)
            label.close()
